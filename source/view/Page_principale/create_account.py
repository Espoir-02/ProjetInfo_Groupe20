...
class CreationCompte_view:
    ...
    def display(self):
        while True:
            questions = [
                ...
                inquirer.Password(
                    "confirmer_mot_de_passe", message="Confirmer le mot de passe:"
                ),
                inquirer.List(
                    "type_utilisateur",
                ...
                inquirer.Confirm(
                    "recommencer",
                    message="Voulez-vous recommencer la saisie ?",
                    default=False,
                ),
            ]
    ...
    def creer_compte(self, answers):
        ...
        if self.informations_obligatoires_remplies(
            nom, prenom, pseudo, mot_de_passe, confirmer_mot_de_passe, type_utilisateur
        ):
            if self.nom_et_prenom_valides(nom, prenom):
                if self.pseudo_valide(pseudo):
                    if self.mots_de_passe_correspondent(
