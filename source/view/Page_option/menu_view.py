    ...
    def display(self):
        ...
        while True:
            ...
            if answers["choice"] == "Rechercher un stage":
                ...
                Session().user_type in ["professeur", "eleve", "administrateur"]
            ) and (answers["choice"] == "Accéder à sa liste d'envie"):
                liste_envie_view = ListeEnvieView(id_utilisateur)
                return liste_envie_view.display()

            elif (
                Session().user_type in ["professeur", "eleve", "administrateur"]
            ) and (answers["choice"] == "Modifier ses informations"):
                from source.view.Page_option.maj_utilisateur_view import (
                    MajUtilisateurView,
                )
                ...
                Session().user_type in ["professeur", "eleve", "administrateur"]
            ) and (answers["choice"] == "Déconnexion"):
                ...
                Session().user_type in ["professeur", "eleve", "administrateur"]
            ) and (answers["choice"] == "Voir mes informations"):
                from source.view.Page_option.mes_info import MesInfos

                info = MesInfos()
                return info.display()

            elif (Session().user_type == "eleve") and (
                answers["choice"] == "Accéder à la liste de suggestions du professeur"
            ):
                suggestions_view = SuggestionEleveView(id_eleve)
                return suggestions_view.display()

            elif (Session().user_type == "professeur") and (
                answers["choice"] == "Accéder à la liste d'élèves"
            ):
                liste_eleves_view = ListeElevesView(id_professeur)
                return liste_eleves_view.display()

            elif (Session().user_type == "administrateur") and (
                answers["choice"] == "Accéder aux fonctions administrateur"
            ):
